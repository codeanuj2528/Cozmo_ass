...
| 1.5 | Tier 2 — **Video**, handheld walkthrough clip | `cozmo/pipeline/video.py`, `cozmo/io/video.py` | Runs on the walkthrough clip; whole-flat clip gives one room of about 371 m², the assignment's single-room walk, which LiDAR puts at 26.90 m², gives 339.61 m² | **NOT MET** as a metric product — the video tier runs end-to-end but does not produce a usable metric plan. Metric scale is not solved. |
| 1.6 | Tier 3 — **LiDAR**, depth + poses + intrinsics | `cozmo/pipeline/lidar.py`, `cozmo/io/stray.py`, `cozmo/geometry/` | Long walk: 7 rooms, 31.26 m² (+9%), 8 openings, adjacency 2/6, per-room ceilings 2.56–2.67 m. Assignment's two flat scans give 8 and 7 rooms, and the same plans from freshly unzipped copies. | **MET** — the strongest tier. Validated against operator's tape on a real flat. Drift correction keeps 77 of 110 candidate loop closures on the long walk and cuts the pose residual from 0.389 to 0.262. |
...
| 2.10 | Benchmark: multi-room, 3+ rooms and a connector | `data/raw/163f18d3ac`, `DROP_CAPTURES_HERE/01_multiroom_lidar` | Hall, passage, bedroom, bathroom, walked twice. 7 rooms on the long walk, 4 rooms on the first walk. | **MET** — two LiDAR walks of the same flat, plus a solo bedroom scan. |
...
| 2.13 | Benchmark: one room captured twice at the same tier | `ae3edc814d`, `163f18d3ac`, `5621ec5c54` | Two LiDAR walks of the flat and one of the bedroom alone. | **MET** as captures — both repeatability pairs fail (0/27 walls agree, ceiling spread 13.1 cm; the bedroom alone against the long walk, 1/6 walls and 0.1 cm). |
...
| 2.17 | Gate: opening widths ≤2 cm on ≥85%, detection scored | `cozmo/bench/gates.py::gate_opening_widths` | Misses and phantoms both counted; reported widths matched to taped ones by the assignment with the smallest total error. 8 openings detected on the long walk. | **UNVERIFIED** — no door tape exists. The gate reports SKIP. |
| 2.18 | Gate: ceiling height ≤1.5 cm per room | `cozmo/bench/gates.py::gate_ceiling_height` | 2.56–2.67 m in 5 of 7 rooms on the long walk. | **UNVERIFIED** — no ceiling tape. The gate reports SKIP. |
| 2.19 | Gate: repeat ceiling spread ≤1 cm, and say which failure | `cozmo/bench/gates.py::gate_repeatability`, `benchmark_report.md` | Walks: hall 0.4 cm, bedroom 0.8 cm, bathroom 13.1 cm; the bedroom alone against the long walk, 0.1 cm. The bathroom fails because the long walk's bathroom room also holds part of the passage. | **FAIL** — ceiling height repeats to 8 mm or better where rooms are segmented the same way (hall, bedroom). Fails in the bathroom, where they are not. |
...
| 2.21 | Gate: drift accountability with an on/off ablation | `cozmo/geometry/drift.py`, `known_failure_modes.md` §5 | Pose graph over heading and horizontal position with ICP-verified closures; four-way ablation (drift on/off × snap on/off), regenerated. With snapping on, the walk without correction loses the passage. | **MET** — the brief makes "poses used as-is" an automatic fail, and the ablation is reported as it came out: this walk drifted little (largest correction 8.5 cm), and the published plan is closest per taped room (26.5% mean error) but not on footprint (+9% against +1% and −3% with snapping off), because it carries three rooms outside the tape. |
...
| 5.3 | Tests exist and pass | `tests/` | 129 tests across 17 test files, all passing. Tests cover schema validation, geometry algorithms, pipeline stages, damage detection, benchmarking, and fixtures. | **MET** |
...
| D5 | Benchmark report across three tiers | `benchmark_report.md`, `reports/verified/gates/` | **PARTIAL** — 14 PASS, 19 FAIL, 33 SKIP. The honest headline is that 33 gates are SKIP because the tape doesn't cover ceilings, doors or the assignment property. |
