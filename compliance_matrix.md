...
| 2.17 | Gate: opening widths ≤2 cm on ≥85%, detection scored | `cozmo/bench/gates.py::gate_opening_widths` | Misses and phantoms both counted; reported widths matched to taped ones by the assignment with the smallest total error. 7 openings detected on the long walk. On the ray-traced room, against its exact dimensions: 2/2 found, no phantom, worst 1.0 cm. | **UNVERIFIED** — no door tape exists, so the gate reports SKIP on every real capture. It passes on the ray-traced room, which is noiseless and shows only that the width is measured without bias. |
| 2.18 | Gate: ceiling height ≤1.5 cm per room | `cozmo/bench/gates.py::gate_ceiling_height` | 2.56–2.67 m in 5 of 6 rooms on the long walk. The ray-traced room reads 2.499 m against 2.50 m, interval [2.486, 2.513]. | **UNVERIFIED** — no ceiling tape, so the gate reports SKIP on every real capture. It passes on the ray-traced room; without its upward lap that room reports the ceiling unmeasured, still SKIP. |
...
| 2.22 | Gate: photo-tier whole-property stitch, ±8% | `cozmo/stitch/rooms.py`, `cozmo/bench/tiers.py` | The assignment's walks against their LiDAR plans: +4.3% PASS, +8.8% FAIL, +120.1% FAIL, no room overlapping another. Home flat, before round 4: +238% against the tape. | **PARTIAL** — one pass in three, by room errors that cancel (−40% and +98% in the plan that passes). Walls within 8%: 4/23, 4/31, 4/30. |
